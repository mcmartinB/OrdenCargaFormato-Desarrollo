...
uses
  ...
  UFImportarOrdenes, uTipos, dmImportarOrdenes;
...
type
  ...
  private
    ...
    procedure SincronizarEstadoAurora(cabifx : TDataset);
...
procedure TFOrdenCarga.SincronizarEstadoAurora(cabifx : TDataset);
var
  dmAurora : TFDMImportarOrdenes;
  cabaurora : TDataset;
begin
  try
    try
    cabAurora := dmAurora.AbrirOrdenCargaAurora(cabifx.fieldbyname('empresa_occ').asstring,
            cabifx.fieldbyname('centro_salida_occ').asstring,
            cabifx.fieldbyname('fecha_occ').asdatetime,
            cabifx.fieldbyname('orden_occ').asinteger);

    dmAurora.CambiarEstadoAurora(cabAurora, cabIfx.fieldbyname('status_occ').asinteger);
    CabAurora.close;
    except
      On E : Exception do
      begin
      dmAurora.RegistrarError(cabifx.fieldbyname('empresa_occ').asstring,
            cabifx.fieldbyname('centro_salida_occ').asstring,
            cabifx.fieldbyname('fecha_occ').asdatetime,
            cabifx.fieldbyname('orden_occ').asinteger);
      end;
...
begin
  ...
  begin
    QOrdenCargaC.Post;
    self.SincronizarEstadoAurora(qOrdenCargaC);
    ActualizarEstado( kVisualizar );
  ...
  begin
    ...
    begin
      ShowMessage('Para los clientes ''GOM'', ''THA'', ''M&W'', ''APS'' y ''EEN'' la base del albaran final son las lineas de la orden y no el paking list como al resto de clientes.' + #13 + #10 +
                  'Por favor revise los calibres del albarán final.' );
      APacking:= True;
...
begin
  ...
  try
    try
    empresa_occ := QOrdenCargaC.FieldByName('empresa_occ').asstring;
    centro_occ := QOrdenCargaC.FieldByName('centro_salida_occ').asstring;
    fecha_occ := QOrdenCargaC.FieldByName('fecha_occ').asDateTime;
    orden_occ := QOrdenCargaC.FieldByName('orden_occ').asinteger;

    dmAurora := TFDMImportarOrdenes.Create(Self, DOrdenCarga.Database,self.CentroDTO);
    registro := dmAurora.AbrirOrdenCargaAurora(empresa_occ, centro_occ, fecha_occ, orden_occ);

     dmAurora.BorrarOccAurora(registro);

     registro.close;
    except
      On E : Exception do
      begin
        dmAurora.RegistrarError(empresa_occ, centro_occ, fecha_occ, orden_occ);
      end;
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          nota_occ.Lines.Add('TEMPERATURA ENTRE 1º y 6º C');
          if cliente_sal_occ.Text = 'SOC' then
            nota_occ.Lines.Add('TEMPERATURA SALIDA PRODUCTO INFERIOR A +4º C');
        end
        else
        ...
        begin
          nota_occ.Lines.Add('TEMPERATURA ENTRE 1º y 8º C');
          nota_occ.Lines.Add('TEMPERATURA SALIDA PRODUCTO INFERIOR A +4º C');
        end
        else
        begin
      ...
      begin
        ...
        begin
          if ( cliente_sal_occ.Text = 'GOM' ) then
            nota_occ.Lines.Add('TEMPERATURA DEL PRODUCTO A LA CARGA ENTRE 7 Y 11ºC.')
          else
          if ( cliente_sal_occ.Text = 'M&W' ) then
            nota_occ.Lines.Add('TEMPERATURA DEL PRODUCTO A LA CARGA ENTRE 7 Y 10ºC.')
          else
          if ( cliente_sal_occ.Text = 'EEN' ) then
            nota_occ.Lines.Add('TEMPERATURA DEL PRODUCTO A LA CARGA ENTRE 6 Y 10ºC.');
          nota_occ.Lines.Add('MANTENER TEMPERATURA ENTRE 8 Y 10 GRADOS.');
          //nota_occ.Lines.Add('TEMPERATURA ÓPTIMA 8 GRADOS.');
...
var
  FImportarDlg : TFImportarOrdenes;
begin
  try
    FImportarDlg := TFImportarOrdenes.Create(Self, DOrdenCarga.Database,self.CentroDTO);
    FImportarDlg.ShowModal;
  finally
    FreeAndNil(FImportarDlg);
  end;
end;
...
begin
  ...
  begin
    if not DDescripciones.EsProductoAlta( producto_ocl.Text) then
      ShowMessage(' ATENCIÓN: Error al grabar la linea, el producto está dado de BAJA. ')
    else if not DDescripciones.EsProductoVenta(producto_ocl.Text) then
      ShowMessage(' ATENCIÓN: Error al grabar la linea, el producto NO está marcado como Producto de Venta. ')
    else
  ...
  begin
    ShowMessage('Falta el código del envase o este es incorrecto.');
    if formato_cliente_ocl.CanFocus then
      formato_cliente_ocl.SetFocus;
    Abort;
  end;
...
begin
  case QOrdenCargaCtraspasada_occ.AsInteger of
    1:
    begin
      lblTranspasada.Caption:= 'ALBARÁN CREADO Nº ' + QOrdenCargaCn_albaran_occ.AsString;
    end;
    2:
    begin
      lblTranspasada.Caption:= 'TRÁNSITO CREADO Nº ' + QOrdenCargaCn_albaran_occ.AsString;
    end;
    else
    begin
