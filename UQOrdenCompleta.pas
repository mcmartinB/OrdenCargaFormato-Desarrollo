...
type
  TQOrdenCompleta = class(TQuickRep)
    ...
    procedure bndPiePackingLineaBeforePrint(Sender: TQRCustomBand;
      var PrintBand: Boolean);
...
function GetDateTime( const AFecha: TDate; const AHora: string ): TDateTime;
var
  iPos: integer;
  sHora, sMinuto: String;
begin
  iPos:= Pos( ':', AHora );
  sHora:= copy( AHora, 0, (iPos-1) );
  sMinuto:= copy( AHora, iPos+1, 2 );
  iPos:= Pos( ':', sMinuto );
  if iPos > 0 then
  begin
    sMinuto:= copy( sMinuto, 0, (iPos-1) );
  end;
  result:= AFecha + EncodeTime( StrToIntDef( sHora, 0 ), StrToIntDef( sMinuto, 0 ), 0, 0 );
end;
...
begin
  ...
  try
    ...
    begin
      ...
      try
        QOrden.bndCliente.Enabled:= DListados.QCab.FieldByName('centro_destino_occ').AsString = '';
...
procedure TQOrdenCompleta.QuickRepBeforePrint(Sender: TCustomQuickRep;
  var PrintReport: Boolean);
begin
  dMax:= StrToDate('1/1/1001');
  dMin:= StrToDate('1/1/3001');
...
begin
  Value:= DataSet.FieldByName('planta_destino_occ').AsString + '-' + Value;
end;

procedure TQOrdenCompleta.QRDBText27Print(sender: TObject;
  var Value: String);
begin
  Value:= DDescripciones.desCentro( DataSet.FieldByName('planta_destino_occ').AsString, Value );
end;
...
begin
  Value:= Value + ' - ' + DDescripciones.desEnvaseP( DataSet.FieldByName('empresa_occ').AsString,
                                                    Value, bndLineas.DataSet.FieldByName('producto_ocl').AsString );
end;
...
procedure TQOrdenCompleta.QRDBText37Print(sender: TObject; var Value: string);
begin
...
procedure TQOrdenCompleta.QRDBText44Print(sender: TObject;
  var Value: String);
begin
  Value:= DDescripciones.desEnvaseProducto( DataSet.FieldByName('empresa_occ').AsString, Value,
                                         bndEnvases.DataSet.FieldByName('producto_pl').AsString );
end;
...
//  if Value = '' then
//    Value:= 'SIN MARCA';
  if bndLineas.DataSet.Active then
    Value:= DDescripciones.desCategoria2( DataSet.FieldByName('empresa_occ').AsString,
                                       bndLineas.DataSet.FieldByName('producto_ocl').AsString,
                                       Value)
  else
    Value := '';
end;
...
procedure TQOrdenCompleta.bndPiePackingLineaBeforePrint(
  Sender: TQRCustomBand; var PrintBand: Boolean);
var
  dAux: TDateTime;
begin
  dAux:= GetDateTime( bndPackingLinea.DataSet.FieldByName('fecha_carga_pl').AsDateTime,
               bndPackingLinea.DataSet.FieldByName('hora_carga_pl').Asstring );
  if dAux > dMax then
  begin
    dMax:= dAux;
  end;
...
procedure TQOrdenCompleta.QRLabel14Print(sender: TObject;
  var Value: String);
begin
  Value:= 'CARGA MIN:' + FormatDateTime('dd/mm/yy-hh:mm', dMin) +
          ' MAX:' + FormatDateTime('dd/mm/yy-hh:mm', dMax);
end;
